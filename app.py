...
def load_history():
    try:
        df = conn.read(worksheet="historial", ttl=15)
...
def load_estado():
    try:
        df = conn.read(worksheet="estado", ttl=15)
        row = df.iloc[0]
...
def save_estado(idx, order):
    conn.update(worksheet="estado", data=pd.DataFrame([{
        "turn_index": int(idx), "order": ",".join(order)
    }]))
...
# 🔔 Mensaje flotante tras una acción (toast)
...
turn_index, MEMBERS = load_estado()
next_person = MEMBERS[turn_index % len(MEMBERS)]
history = hist_df.to_dict("records") if not hist_df.empty else []

# ================================================================
# ============  MODAL: corregir / eliminar una RQ  ===============
# ================================================================
@st.dialog("✏️ Corregir o eliminar una RQ")
def dialog_corregir(hist):
    if not hist:
        st.write("Aún no hay RQ's para corregir.")
        return
    opciones = [f"{i+1}. {h['time']} · {h['member']} · {h['client']} · {h['priority']}"
                for i, h in enumerate(hist[:MAX_SHOWN])]
    ...
    nueva_prio = ec2.selectbox("Prioridad", PRIORITIES,
    ...
    if b1.button("💾 Guardar corrección", use_container_width=True):
        ...
        if fila is None:
            ...
            st.session_state.flash = "✅ RQ corregida"
            st.rerun()
    if b2.button("🗑️ Eliminar esta RQ", use_container_width=True):
        ...
        if fila is None:
            ...
            save_history(fresh); clear_cache()
            st.session_state.flash = "🗑️ RQ eliminada"
            st.rerun()

# ================================================================
# ============  FRAGMENT: editor de orden (rápido)  ==============
# ================================================================
@st.fragment
def editor_orden(current_turn, members):
    st.caption("Usa las flechas para mover a cada persona. Al terminar, pulsa **Guardar orden**.")
    ...
    for i, m in enumerate(draft):
        ...
        c_up.button("⬆️", key=f"up_{i}", disabled=(i == 0), on_click=mover, args=(i, "up"), use_container_width=True)
        c_down.button("⬇️", key=f"down_{i}", disabled=(i == len(draft)-1), on_click=mover, args=(i, "down"), use_container_width=True)
...
with head_col2:
    ...
    if st.button("🔄 Actualizar", use_container_width=True, help="Trae los últimos cambios."):
        clear_cache()
        st.rerun()
...
with col2:
    if history:
        ...
        last_inner = (
            ...
            f'<span class="meta-text">por {l["member"]} · {l["time"]}</span></div>'
        )
    else:
        last_inner = '<span class="empty">Aún no hay registros</span>'
    st.markdown(H(f'<div class="card"><div class="label">Última RQ tomada</div>{last_inner}</div>'), unsafe_allow_html=True)
    ...
    with st.form("nueva_rq"):
        ...
        if submitted:
            ...
            new_row = pd.DataFrame([{
                ...
                "priority": priority, "time": fmt_time(ahora_bogota())
            }])
            updated = pd.concat([new_row, fresh], ignore_index=True)
            save_history(updated)
            save_estado((turn_index + 1) % len(MEMBERS), MEMBERS)
            clear_cache()
            st.session_state.flash = f"✅ RQ de {client} registrada para {next_person}"
            st.rerun()

# ---------------- Acciones: editar orden / corregir ----------------
acc1, acc2 = st.columns(2)
with acc1:
    with st.expander("⚙️ Editar orden de rotación"):
        editor_orden(turn_index, MEMBERS)
with acc2:
    ...
    if history:
        for h in history[:MAX_SHOWN]:
            block += (
                ...
                f'<span class="hist-time">{h["time"]}</span></div>'
            )
    else:
